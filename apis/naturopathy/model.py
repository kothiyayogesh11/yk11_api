    ...
    if find_outlet:
        if requestData['naturopathy_id'] == "":
            ...
            if requestData["sub_category_id"] == "other":
                ...
                requestData["sub_category_id"] = ObjectId(requestData["sub_category_id"])
            # requestData["sub_category_id"] = ObjectId(requestData["sub_category_id"])
            requestData['naturopathy_id'] = ObjectId(requestData['naturopathy_id'])
            ...
            act = DB.update_one(tbl_v028_meditation_naturopathy,requestData,{"_id":requestData["naturopathy_id"]})
            if act:
                code = 200
                status = True
...
def naturopathy_list(self,outlet_id):
    code = 201
    status = False
    message = ""
    responseData = {}
    ...
    fieldsList = {"_id":1,"outlet_id":1,"title" : 1,"description" : 1,"date":1,"type":1}
    if outlet_id != "":
        get_all_Data = DB.find_using_aggregate(tbl_v028_meditation_naturopathy,[{"$match":{"outlet_id":ObjectId(outlet_id),"is_delete":0,"business_type":"naturopathy"}},{"$sort":{"_id":-1}},{"$project":fieldsList}])
    else:
        get_all_Data = []
    if(get_all_Data):
        code = 200
    ...
    logging.debug('naturopathy_list: {}'.format(response))
    return response


def remove_naturopathy(self):
    code = 201
    status = False
    message = ""
    requestData = dict(request.json)
    ...
    todayDate = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ip_address = socket.gethostbyname(socket.gethostname())
    find_naturopathy_id = DB.find_one(tbl_v028_meditation_naturopathy,{"_id":ObjectId(requestData['naturopathy_id'])},"ALL")
    
    if find_naturopathy_id:
        delete_naturopathy_id = DB.update_one(tbl_v028_meditation_naturopathy,{"is_delete":1},{"_id":ObjectId(requestData['naturopathy_id'])})
        if delete_naturopathy_id:
            code = 200
            status = True
            message = MSG_CONST.NATUROPATHY_DELETED_SUCCESS
        else:
            code = 201
            status = False
            message = MSG_CONST.NATUROPATHY_DELETE_FAILED
    else:
        code = 201
        status = False
        message = MSG_CONST.NATUROPATHY_NOT_FOUND   
        
    response = output_json(responseData,message,status,code)
    logging.debug('remove_naturopathy: {}'.format(response))
    return response
...
def category_list_for_service(self):
    ...
    if vendor_id:
        ...
        categoryData = DB.find_by_key(tbl_v006_service_category,{"type" : "naturopathy"},{"_id":1,"category_name":1})
        if categoryData:
            resData = []
...
def getNaturopathy(self):
    code = 200
    status = False
    message = ""
    responseData = {}
    # where = {}
    find_naturopathy_arr={}
    if request.args.get("naturopathy_id") and request.args.get("naturopathy_id") != "":
        naturopathy_id = ObjectId(request.args.get("naturopathy_id")) 
        find_naturopathy = DB.find_one(tbl_v028_meditation_naturopathy,{"_id":naturopathy_id},'ALL')
        find_cat = DB.find_all_where(tbl_v007_services_master,{"category_id":ObjectId(find_naturopathy['category_id']['$oid'])},'ALL')
        accomodation_title = find_naturopathy['accomodations']
        if(find_naturopathy):
            find_naturopathy_arr['data']=find_naturopathy
            for x in accomodation_title:
                title_id = x['accomodation_id']
                find_title = DB.find_one(tbl_v023_accomodation,{"_id":ObjectId(title_id)},{"title":1,'_id':0})
                x["title"] = find_title["title"]
            find_naturopathy_arr['cat']=find_cat            
            code = 200
            status = True
            message = MSG_CONST.VENDOR_SERVICES_SUCCESS
        else:
        ...
        status = False
        message = "something went wrong"
    response = output_json(find_naturopathy_arr,message,status,code)
    logging.debug('vendor_services_list: {}'.format(response))
    return response 
...
def getMasterServiceByCategory(self):
    ...
    if category_id:
        serviceData = DB.find_by_key(tbl_v007_services_master,{"category_id":{"$in":category_id}},{"_id":1,"name":1})
    else:
        serviceData = {}
