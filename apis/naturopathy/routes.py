...
ns = Namespace('naturopathy', description='Naturopathy Related related operations')
...
@ns.route('/naturopathy_list_by_vendor')
class NaturopathyListByVendor(Resource):
    @ns.response(201, 'Get All vendor sevices list.')
    @ns.doc('Vendor - Get vendor sevices list')
    def get(self):
        """ Get Services By Vendor """
        return getNaturopathyByVendor(self)
...
@ns.route('/remove_naturopathy')
class DeleteNaturopathy(Resource):
    @ns.response(201, 'delete meditation.')
    @ns.doc('Vendor - Login')
    def post(self):
        """Vendor Login API """
        return remove_naturopathy(self)

@ns.route('/get_all_naturopathy')
class GetAllNaturopathy(Resource):
    @ns.response(201, 'get all naturopathy.')
    @ns.doc('Vendor - Login')
    def get(self):
        """Vendor Login API """
        outlet_id =""
        ...
        return naturopathy_list(self,outlet_id)    
